...
### Step 2: Fire up Docker
```bash
$ docker compose run --service-ports ray-feast
```

From inside the container CLI, run the next steps!

### Step 3: Register and upload features to Redis

To initialize the feature store you can run

```bash
root@527ed60c62cd:/app# make init-fs
```
...
```bash
root@527ed60c62cd:/app# make test-fs
```
...
```bash
root@527ed60c62cd:/app# make train
```
...
```bash
root@527ed60c62cd:/app# make train-ray
```
...
```bash
root@527ed60c62cd:/app# EXPORT RAY_ADDRESS=<address of Ray head node>:<ray head port>
```
...
```bash
root@527ed60c62cd:/app# make serve
```
...
```bash
root@527ed60c62cd:/app# make test-infer
```
